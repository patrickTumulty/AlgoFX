...
public class ArrayComponent<T extends Number> implements ArrayModel.Listener<T>
{
    ...
    {
        int width = (maxArrayWidth / model.size()) - (((model.size() - 1) * rectangleSpacing) / model.size());
    ...
    private void updateRectangle(int i, T value)
    {
        if (rectangleMap.containsKey(i))
        {
            ...
            FxUtils.run(() -> rectangle.setHeight(calculateRelativeRectangleHeight(value.floatValue())));
        }
    }
    ...
    {
        Optional.ofNullable(rectangleMap.get(i)).ifPresent(rectangle ->
        {
            clearSelected();
